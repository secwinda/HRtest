...
mb.vi.wfCardView.prototype.getFormatMenus = function() {
	...
	$.each(_nextData, function(i, actItem) {
		...
		var eventName = UIConst.EVENT_PRE + actItem.NODE_CODE;
		menuArr.push({
			"eventName":eventName,
	...
	$.each(_buttonData, function(i, actItem) {
		var filterStr = ",save,delete,finish,cmSimpleFenFa,toShouwen,cmTuiHui,cmQianShou,stopParallelWf,withdraw,cmWfTracking,"; //需要显示的操作类按钮
		if (_self._parHandler 
				&&_self._parHandler._formData 
				&& _self._parHandler._formData.authBean 
				&& _self._parHandler._formData.authBean.userDoInWf == "true") { //当前办理人
		} else {
			filterStr = ",withdraw,undoFinish,cmQianShou,cmSimpleFenFa,cmWfTracking,"; //收回，取消办结，阅毕，简单分发
		}
		
		console.log(actItem.ACT_CODE);
		if (!actItem.ACT_CODE || filterStr.indexOf(actItem.ACT_CODE) < 0) {
			return true; //继续下一次循环
		}
...
mb.vi.wfCardView.prototype._initMind = function() {
	...
	var mindInputFieldCon = $("<div><input readonly='readonly' placeholder='我的意见...' /></div>"); // 意见输入框Field
