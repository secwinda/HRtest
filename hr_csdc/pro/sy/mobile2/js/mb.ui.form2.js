...
mb.ui.form2.prototype._bldLayout = function() {
	// 给页面添加一个头
	this._pCon.append("<div style='height:2em;line-height:2em;text-align:center;color:red;text-shadow:none;font-weight:bold;font-size:1.2em;'>" + this.opts.parHandler.servName + "</div><div class='mind-divider'></div>");
...
mb.ui.form2.prototype._bindEvent = function() {
	...
	*/
	...
	_self.mainContainer.on('vclick', '.js-file-popup', function(event) {
		event.preventDefault();
		var fileId = $(this).attr('data-file-id');
		var eventTarget = $(this);
		var fileName = eventTarget.parent().find('h4').text();
		...
		editBtn.on('vclick', function(event) {
			event.preventDefault();
			// 构建文件信息修改页面
			var fileEditDia = $("<div id='file-edit-dia' data-role='dialog'></div>");
...
//					"</div>" +
			...
			$("<a href='#' data-role='button' data-rel='back'>取消</a>").appendTo(fileEditDia);
			$("<a href='#' data-role='button'>确定</a>").appendTo(fileEditDia).on('click', function() {
				console.log('TODO修改文件名字,返回页面,刷新');
			});
			...
			$('#file-update').popup('close');
		});
		deleteBtn.on('vclick', function(event) {
			event.preventDefault();
			console.log('TODO删除当前fileId的文件,然后刷新页面');
		});
	...
	}).on('vclick', '.js-ui-date', function() {
		var eventTarget = this;
		var ui = _self._uItems[$(eventTarget).attr('itemCode')];
		if (ui.readOnly == 1) {
			return;
		}
		var params = {
				'dateMode': 'DATE',
				'initDate': '2015-05-08 20:21:22',
				'minDate': '2015-01-08',
				'maxDate': '2016-01-08'
		};
...
mb.ui.selectMenu = function(options) {
	...
	var group = $("<select data-native-menu='false'></select>").attr('id', opts.id).attr('name', opts.id);
	if (multiple) {
		group.attr('multiple', 'multiple');
	}
//	$("<option>请选择</option>").appendTo(group);
...
mb.ui.file = function(options) {
	...
	this.obj = $("<div class='ui-input-text ui-body-inherit ui-corner-all ui-shadow-inset'><span class='ui-btn ui-icon-carat-r ui-btn-icon-right'>点击进入列表</span></div>");
};
...
mb.ui.attach = function(options) {
	...
	var group = $("<ul data-role='listview' data-inset='true' style='padding:0 0 .5em 0;'></ul>");
	this.obj = group;
};
mb.ui.attach.prototype.setValue = function(fileData) {
	...
	for (var i=0; i<fileData.length; i++) {
		...
		var fileName = fileItem['FILE_NAME'];
		var fileId = fileItem['FILE_ID'];
		fileIds.push(fileId);
		
		if (fileItem['FILE_CAT'] == this.itemCode) {
			hasFlag = true;
			
			$("<li id='" + fileId + "'><a href='#' class='js-file-popup' data-rel='popup' data-file-id=''><h4>" + fileItem['DIS_NAME'] + "</h4></a>" +
