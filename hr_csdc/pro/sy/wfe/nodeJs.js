...
//    jQuery("#CUSTOM_SEND_FLAG").click(function(){
//    	if(jQuery(this).attr("checked")){
//    		jQuery("#CUSTOM_SEND_CONFIG").val("{'NODE_CODE':'N1','BTN_NAME':'退回发起人'}");
//    	}else{
//    		jQuery("#CUSTOM_SEND_CONFIG").val("");
//    	}
//    });
    
	  jQuery("#RTN_QCR_FLAG").click(function(){
		  if(jQuery(this).attr("checked")){
			  jQuery("#RTN_QCR_NAME").val("退回发起人");
		  }else{
			  jQuery("#RTN_QCR_NAME").val("");
		  }
	  });
    ...
    //初始化扩展条件
    customBl.init(nodeObj.WF_CUSTOM_BL);
	...
	//treeData[0].CHILD.splice(1,2);表单变量
	treeData[0].CHILD.splice(1,1);//意见变量
	treeData[0].CHILD.shift();//删除第一项,流程变量
	
	jQuery("#extmodel1").click(function() {
		$("input[name='NODE_EXTEND_CLASS']").val("com.rh.core.wfe.resource.CommExtBinder");
		$("#customItem").show();//显示div  
	});
	jQuery("#extmodel2").click(function() {
		$("input[name='NODE_EXTEND_CLASS']").val("");
		$("#customItem").hide();//隐藏div  
	});
});
...
function initBaseInfo(nodeObj){
	...
	//$("#CUSTOM_SEND_CONFIG").val(nodeObj.CUSTOM_SEND_CONFIG);
	$("#RTN_QCR_NAME").val(nodeObj.RTN_QCR_NAME);
	...
	$("#PEOPLE_LEVEL").val(nodeObj.PEOPLE_LEVEL);
...
//	$("CUSTOM_SEND_FLAG").val(nodeObj.CUSTOM_SEND_FLAG);
//	if(nodeObj.CUSTOM_SEND_FLAG=="1"){
//		$("#CUSTOM_SEND_FLAG").attr("checked",true);
//	}
	$("RTN_QCR_FLAG").val(nodeObj.RTN_QCR_FLAG);
	if(nodeObj.RTN_QCR_FLAG=="1"){
		$("#RTN_QCR_FLAG").attr("checked",true);
	}
	/** 给是否自动结束给一个默认自动结束的值*/
	...
	if("com.rh.core.wfe.resource.CommExtBinder"== nodeObj.NODE_EXTEND_CLASS){
		$("#extmodel1").attr('checked','true');
	} else {
		$("#extmodel2").attr('checked','true');
	}
}
...
function confirmall(closeWin){
	var chkboxIds = ["PROC_END_FLAG","NODE_IF_PARALLEL","NODE_IF_CONVERGE","MIND_NEED_FLAG"
	                 ,"MIND_REGULAR_MUST","MIND_TERMINAL_MUST","NODE_IF_AUTOEND","RTN_QCR_FLAG"];
    ...
    var nodeObj = {  
		...
		//CUSTOM_SEND_FLAG : $("#CUSTOM_SEND_FLAG").val(),	
		//CUSTOM_SEND_CONFIG : $("#CUSTOM_SEND_CONFIG").val(),
		RTN_QCR_FLAG : $("#RTN_QCR_FLAG").val(),	
		RTN_QCR_NAME : $("#RTN_QCR_NAME").val(),	
		...
		WF_CUSTOM_BL : customBl.saveDef(),
		NODE_DEF_VERSION : NODE_DEF_VERSION,
		PEOPLE_LEVEL : $("#PEOPLE_LEVEL").val()
    };
    ...
    window.parent.updateNodeDef(nodeObj);
 ...
 * 打开部门选择和角色选择的树
 */
function openTreeDialogDept(inputName, radioInputId,flag) {
	    var configStr = "SY_ORG_DEPT, {'TYPE':'multi'}";
		if(flag == 1) {
			configStr = "SY_ORG_DEPT_ALL, {'TYPE':'multi'}";
		}
...
var customBl = {
	init : function(defVal) {
		jQuery(document).ready(function() {
			var nodeCK = $("input[name='extmodel']:checked").val();
			if(nodeCK==2){
				$("#customItem").hide();
			}
			jQuery("#btnCustomBlAdd").click(function() {
				blcount++;
				customBl.addVar();
			});
		});
		...
		if(defVal){
			...
			if(typeof(defVal) == 'string'){
				try{
					...
					jQuery(list).each(function(index,obj){
						blcount++;
						customBl.addVar(obj.BL_CODE,obj.BL_CODENAME,obj.BL_CAOZUO,obj.BL_VALUE,obj.BL_ROLE,obj.BL_ROLENAME,obj.BL_USER,obj.BL_USERNAME,obj.BL_LUOJI);
					});
				}catch(e){
	...
	addVar: function(blCode,blCodeName,blCaoZuo,blValue,blRole,blRoleName,blUser,blUserName,blLuoJi) {
		blCode = blCode || "";
		blCodeName = blCodeName || "";
		blCaoZuo = blCaoZuo || "";
		blValue = blValue || "";
		blRole = blRole || "";
		blRoleName = blRoleName || "";
		blUser = blUser || "";
		blUserName = blUserName || "";
		blLuoJi = blLuoJi || "";
		var rows = "<tr class='blList'><td>"
				 + "<textarea name='BL_CODE__NAME' id='BL_CODE"+blcount+"__NAME' class='nwp' readonly rows='2' ondblclick=openSysParams('BL_CODE"+blcount+"')>" + blCodeName + "</textarea>"
				 + "<input type='hidden'  name='BL_CODE' id='BL_CODE"+blcount+"' value="+blCode+"> "
				 + "</td><td>"
				 + "<select name='BL_CAOZUO' id='BL_CAOZUO"+blcount+"'>" +
				 		...
				 		"</select>"
				 + "</td><td>"
				 + "<textarea name='BL_VALUE' id='BL_VALUE"+blcount+"' class='nwp' rows='2'>" + blValue + "</textarea>"
				 + "</td><td>"
				 + "<textarea name='BL_ROLE__NAME' id='BL_ROLE"+blcount+"__NAME' class='nwp' readonly rows='2' ondblclick=openTreeDialog('BL_ROLE"+blcount+"','SY_ORG_ROLE','NODE_ROLE_MODE_1')>" + blRoleName + "</textarea>"
				 + "<input type='hidden'  name='BL_ROLE' id='BL_ROLE"+blcount+"' value="+blRole+"> "
				 + "</td><td>"
				 + "<textarea name='BL_USER__NAME' id='BL_USER"+blcount+"__NAME' class='nwp' readonly rows='2' ondblclick=openTreeDialog('BL_USER"+blcount+"','SY_ORG_DEPT_USER_ALL','NODE_USER_MODE_1')>" + blUserName + "</textarea>"
				 + "<input type='hidden'  name='BL_USER' id='BL_USER"+blcount+"' value="+blUser+"> "
				 + "</td><td>"
				 + "<select name='BL_LUOJI' id='BL_LUOJI"+blcount+"'>" +
				 		"<option value=''></option>" +
				 		"<option value='START'>开始</option>" +
				 		"<option value='DO'>+</option>" +
				 		"<option value='END'>结束</option>" +
				 		"</select>"
				 + "<input type='hidden' name='BL_SORT' id='BL_SORT"+blcount+"' value="+blcount+">"
				 + "</td><td onclick='customBl.removeVar(this)'><a>删除</a></td></tr>";
		
		jQuery("#tblCustomBlAdd").append(rows);
		jQuery("#tblCustomBlAdd").find("select[id=BL_CAOZUO"+blcount+"]").val(blCaoZuo);
		jQuery("#tblCustomBlAdd").find("select[id=BL_LUOJI"+blcount+"]").val(blLuoJi);
	},
	/**删除指定变量**/
	removeVar : function(obj){
		jQuery(obj).parent().remove();
	...
	saveDef : function(){
		var def = new Array();
		jQuery("#tblCustomBlAdd").find("tr.blList").each(function(index,obj){
			var item = {
				BL_CODE : jQuery(obj).find("input[name=BL_CODE]").val(),
				BL_CODENAME : jQuery(obj).find("textarea[name=BL_CODE__NAME]").val(),
				BL_CAOZUO : jQuery(obj).find("select[name=BL_CAOZUO]").val(),
				BL_VALUE : jQuery(obj).find("textarea[name=BL_VALUE]").val(),
				BL_ROLE : jQuery(obj).find("input[name=BL_ROLE]").val(),
				BL_ROLENAME : jQuery(obj).find("textarea[name=BL_ROLE__NAME]").val(),
				BL_USER : jQuery(obj).find("input[name=BL_USER]").val(),
				BL_USERNAME : jQuery(obj).find("textarea[name=BL_USER__NAME]").val(),
				BL_LUOJI : jQuery(obj).find("select[name=BL_LUOJI]").val(),
				BL_SORT : jQuery(obj).find("input[name=BL_SORT]").val()
			};
			def.push(item);
		});
		def = Hex.encode(jQuery.toJSON(def))
		return def;
	}
};
 ...
 * 打开选择系统变量的树
 */
function openSysParams(inputName) {
    var _self = this;
	...
	var configStr = "SY_ORG_USER" + ",{'TYPE':'multi'}";
	...
	"replaceCallBack":function(id,value){
		...
		jQuery("#" + inputName+ "__NAME").val(value);
	}};
	var dictView = new rh.vi.rhDictTreeView(options);
	dictView.show(event);
}
