...
rh.cm.mind.prototype.changeGeneralMind = function() {
	...
	if (generalMindDef && generalMindDef.CODE_ID) {
		...
		var currInputVal = _self._generalMind.mindTextInput.val();
		(_self._oldGeneralMindDef["DEFAULT_CONTENT"] == currInputVal) && (currInputVal = "");
		//initInput
		_self._generalMind.file && _self._generalMind.file.destroy();//必须销毁之前的对象
		_self._createGeneralMindFrame(generalMindDef, false);
		//bindInputData
		...
		if (ifOutDept) { // 出部门或者机构了
  			...
  			if (_self._hasRegularMindInput()) { // 固定意见
  				if (_self._hasWriteRegularMind()) { 
	  				...
	  				top.Tip.addTip("请选择" + regularMind.CODE_NAME + "！", "warn");
	  				return false;
	  			}
  			}
  			...
  			if (_self._hasTerminalMindInput()) {
  				if (!_self._hasWriteTerminalMind()) { // 没有填写意见
  					top.Tip.addTip("请填写" + currMind.CODE_NAME + "！", "warn");
			   		return false;
		   		}
  			}
	   	}
...
rh.cm.mind.prototype.addAutoHeight = function(mindTextInput){
	...
	thisTextObj.css({"min-height":"50px", "line-height":"normal","overflow":"hidden"});
