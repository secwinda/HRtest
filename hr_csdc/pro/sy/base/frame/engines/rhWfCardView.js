...
rh.vi.wfCardView.prototype._bldNextBtnBar = function(_nextData) {
	...
	if(_self.btnRenderMode == 0 || _self.btnRenderMode == 100){	//平铺
		var firstBtn = jQuery("a:first", _self._btnBar);
...
rh.vi.wfCardView.prototype.getMindCodeBean = function() {
	...
	for (var i=0; i<genMindList.length; i++) {
		...
		if(!mindScript || eval(mindScript)) { // 如果没有条件，或者条件判断通过
 ...
 */
rh.vi.wfCardView.prototype._openSelectOrg = function(nodeCode, aObj, event, selectedUserIds, selectedNames, excludeUserId) {
	var _self = this;
	...
	function callbackFetchUser(rtnTreeData) {
		...
		if (sendDirectFlag) { // 直接送人
			...
			var extendTreeSetting = "{'rhexpand':false,'expandLevel':3,";

			var selectType = "single";
			var modelType = "default"; //default 区分是因为如果单选，link的能选多个人
			if (multiSelect == "true") {
				selectType = "multi";
				modelType = "link";
				if (_self.btnRenderMode == 12 || _self.btnRenderMode == 112) {
			...
			//添加弹出框默认样式
			var configStr = "SY_ORG_USER" + ",{'TYPE':'" + selectType + "','MODEL':'"+modelType+"','CHECKBOX':" + multiSelect + "}"; // multi
			// ,
			// single
			var options = {
				"itemCode" : inputName,
			...
			/** 是否自动选择最终用户 
			 * autoSelect == 1, 弹出选择人的树，并自动将人选中
			 * autoSelect == 3, 不弹出选人的树，将所有的叶子节点数据返回， 处理完毕模式放入框中； 非处理完毕直接送交
			 * autoSelect == 其他值， 弹出选人的树， 不将人自动选中
			 */
			if (rtnTreeData.autoSelect == "1") { 
				dictView = new rh.vi.rhDictTreeView(options);
				dictView.show(event);
				
				if (!selectedUserIds) { //之前没有被选中过 ， 则全选
			...
			} else if (rtnTreeData.autoSelect == "3") {
				var users = _findAllUsers(treeData);
				var userIds = new Array();
				var userNames = new Array();
				jQuery(users).each(function(index, obj) {
					if (obj.ERR_MSG) { //如果有错误信息，则不添加到送交的里面去了
						top.Tip.addTip(obj.ERR_MSG, "warn");
					} else {
						userIds.push(obj.ID);
				...
				if (_self.btnRenderMode == 12 || _self.btnRenderMode == 112) { //处理完毕模式
					var toType = "3";
					if (_self.binderType == "ROLE") {
						toType = "1";
						_self.reqdata["TO_DEPT"] = userIds.join(","); // 送交部门
						_self.reqdata["TO_ROLE"] = _self._binderRoleCode; // 送交角色
					} else {
						_self.reqdata["TO_USERS"] = userIds.join(","); // 送交人
					}
					...
					_self.confirmUserSelectSend(_self.reqdata, userNames);
				} else { //非处理完毕模式，直接送交
					_self._confirmSend(userIds, userNames);
				}
			} else {
				dictView = new rh.vi.rhDictTreeView(options);
				dictView.show(event);				
			...
			if (modelType == "link" && selectedUserIds) { //如果是显示右边列表模式，而且在打开选择用户的时候，传入了已经选择的用户
				var selectUserArr = selectedUserIds.split(",");
				var selectUserNameArr = selectedNames.split(",");
				var userSelectObjs = new Array();
				var userIds = new Array(); 
				for (var j=0;j<selectUserArr.length;j++) {
					var userSelect = {};
					userSelect.ID = selectUserArr[j];
					userSelect.NAME = selectUserNameArr[j];
					userSelectObjs.push(userSelect);
					
					userIds.push(userSelect.ID);
				}

				dictView.setRightSelect(userSelectObjs);
				dictView.tree.selectNodes(userIds);
			} else if (selectedUserIds) { //如果单选，而且传递了已经选择的用户，则也默认勾选
				var userIds = new Array();
