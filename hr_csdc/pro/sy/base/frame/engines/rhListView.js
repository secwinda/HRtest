...
rh.vi.listView = function(options) {
	var defaults = {
		...
		"title":"",//列表标题
		"servDef":null, //服务定义数据
		"hideAdvancedSearch":false //是否隐藏高级查询
	};
		    ...
		    taObj.bind("click",function(event) {
		    	...
		    	var importWin = new rh.ui.popPrompt({
		    		...
		    		okFunc:function() {
		    			...
		    			param["fileId"] = fileId;
		    			//提交  导入 只将fileId传入即可
		    			rh_processData(_self.opts.sId + ".imp.do", param, true);
...
rh.vi.listView.prototype._bldBtn = function(actItem) {
	...
	var labelName = jQuery("<span></span>").addClass("rh-icon-inner").text(actItem.ACT_NAME);
	temp.append(labelName);
