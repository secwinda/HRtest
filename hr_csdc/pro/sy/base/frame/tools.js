...
var Tools = {
	...
	itemVarReplaceByCardObj: function(str, cardObj) {
		try {
			if (typeof (cardObj.getItem) === "function") {
				var reg = /#.*?#/g;
				var result = str.match(reg);
				if (result != null) {
					for (i = 0; i < result.length; i++) {// 循环区块
...
//		    	console.log("destroyIframe:" + e.message);
	...
	bodyScroll : function(bool) {
		if (bool) {
			...
			parent.jQuery("html").css({"overflow":""});
		} else {
			parent.jQuery("body").css({"overflow":"hidden"});
			parent.jQuery("html").css({"overflow":"hidden"});
		}
	},
	/**
	...
	openByParams : function(todoParams){
		if (todoParams.url.indexOf(".showDialog.do") > 0 || (todoParams.con && todoParams.con.length > 0)) {
			...
			var options = {"url":todoParams.url, "tTitle":todoParams.title
					, "params":todoParams.params,"areaId":todoParams.areaId};
			Tab.open(options);
		}		
	},
...
var SysMsg = {
		...
		"confirm" : function(msg,yesfunc,nofunc){
			...
			var dlgId = "SysMsg-" + StringUtils.randomNum();
			arr.push("<div" + " id='" + dlgId + "' title='提示'>");
			arr.push("<div class='ml10 mr10 mt30 f16'>" + msg + "</div>");
			arr.push("</div>");
