 ...
 * 定义流程
 */
_viewer.grid.getBtn("oper").unbind("click").bind("click", function(event) {
	var pk = jQuery(this).attr("rowpk");
	var procEnName = _viewer.grid.getRowItemValue(pk, 'EN_NAME');
	var procName = _viewer.grid.getRowItemValue(pk, 'PROC_NAME');
	var cmpy = _viewer.grid.getRowItemValue(pk, 'S_CMPY');
	var servId = _viewer.grid.getRowItemValue(pk, 'SERV_ID');
	var servPid = _viewer.grid.getRowItemValue(pk, 'SERV_PID');
	var param = "procCode="+ pk +"&procEnName=" + procEnName + "&procName=" + encodeURIComponent(procName) + "&cmpy=" + cmpy + "&servId=" + servId + "&servPid=" + servPid;
	//window.open(FireFly.getContextPath() + "/sy/wfe/procDef-gojs.jsp?" + param);
	//event.stopPropagation();
	var opts = {"tTitle":"工作流设计器","url":FireFly.getContextPath() + "/sy/wfe/procDef-gojs.jsp?" + param,"params":{},"menuFlag":3};
	Tab.open(opts);
	return false;
});
